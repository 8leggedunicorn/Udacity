...
\begin{figure}[H]
  ...
  \begin{python}
    ...
    ttest = scipy.stats.ttest_rel(stroopdata['Incongruent'],
                                    stroopdata['Congruent'])
    # As scipy.stats.ttest_rel calculates a two-tailed test to obtain the p value
    # for the one-tailed test requires dividing this p value by 1/2, so for the
    # one-tailed test:
    print("(",ttest[0],",",ttest[1]*0.5,")")
    ( 8.02070694411, 2.05150029286e-08 )
  \end{python}
  \caption{Calculation of the one-tail correlated \textit{t} test shows that the
...
\end{figure}
So to interpret the results.  First we are interested in the one-tail result.
Note that as the \textit{t} Test assumes a symmetric distrobution the $p$ value of
the one-tail test is simply half the value of the two tail test, while the
\textit{t} remains unchanged.  So the first number, $8.02070694411,$ obtained in the last line of
\autoref{fig:reject-null-hypothesis} is the \textit{t} value and the second of
$2.05150029286e-08$ is the $p$ value for the one-tailed correlated
\textit{t} Test.

That is the population mean, $\mu_{ICS}$ for the ICS
data set is greater than the $\mu_{CCS}$ ( $\mathit{t} = 8.020706944109957$ at
$p = 2.05150029286e-08$ ), an exceedingly unlikely occurrence, roughly two in
...
only reasonably acceptable hypothesis, with almost no chance of a Type I
error occurring. 
